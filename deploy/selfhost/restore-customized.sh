...
function restoreSingleVolume() {
    ...
    local PLANE_DOWNLOAD_URL=${PLANE_DOWNLOAD_URL:-"https://github.com/TheAgentCompany/the-agent-company-backup-data/releases/download/plane-20241031-0351"}
    
    docker run --rm \
        -v "$selectedVolume":"/vol" \
        busybox sh -c "wget -O /tmp/${restoreFile}.tar.gz ${PLANE_DOWNLOAD_URL}/${restoreFile}.tar.gz && \
                       mkdir -p /restore && \
                       tar -xzf /tmp/${restoreFile}.tar.gz -C /restore && \
                       mv /restore/${restoreFile}/* /vol/ && \
                       rm -f /tmp/${restoreFile}.tar.gz"
